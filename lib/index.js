...
exports.start = function (options) {
    ...
    Fs.readdir(internals.args.source, (err, files) => {
        ...
        for (var f = 0; f < files.length; ++f) {
            var fileExt = Path.extname(files[f]).toLowerCase();
        ...
        Items.parallel(validFiles, (file, next) => {
            ...
            Fs.readFile(`${internals.args.source}/${file}`, 'utf8', (err, stopwords) => {
                ...
                for (var s = 0; s < stopwords.length; ++s) {
                    var stopword = stopwords[s].trim().toLowerCase();
                    if (stopword) {
                        var tabbed = stopword.split('\t');
                        if (tabbed.length > 1) {
                            tabbed = tabbed.filter(internals.uniq);
                            for (var t = 0; t < tabbed.length; ++t) {
                                var word = tabbed[t].trim().toLowerCase();
